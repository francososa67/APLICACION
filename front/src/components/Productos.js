...
const Productos = () => {
  ...
  const [editProducto, setEditProducto] = useState({ id: null, nombre: '', precio: 0 });
  ...
  const handleAddProducto = () => {
    axios.post('http://localhost:4000/productos', newProducto)
      .then(response => {
  ...
  const handleModifyProducto = (id) => {
    setEditProducto(productos.find(producto => producto.id === id));
  };

  const handleSaveProducto = () => {
    axios.put(`http://localhost:4000/productos/${editProducto.id}`, editProducto)
      .then(response => {
        setProductos(productos.map(producto => 
          producto.id === editProducto.id ? response.data : producto
        ));
        setEditProducto({ id: null, nombre: '', precio: 0 });
      })
          ...
          <li key={producto.id}>
            {editProducto.id === producto.id ? (
              ...
              <>
                ...
                <button onClick={() => handleModifyProducto(producto.id)}>Modificar</button>
              </>
            )}
          </li>
        ))}
